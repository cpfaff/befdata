...
class User < ActiveRecord::Base
  ...
  def datasets_owned
    roles.includes(:authorizable).where(name: 'owner', authorizable_type: 'Dataset').map(&:authorizable).compact
  end

  def datasets_with_responsible_datacolumns
    columns = roles_for(Datacolumn).map(&:authorizable_id)
  ...
  def open_votes_count
    paperproposal_votes.where("vote = 'none'").count
  end
  ...
  def add_protocol_to_url
    if url.present? && url_changed?
      /^http/ =~ url ? url : self.url = "http://#{url}"
    end
  end
