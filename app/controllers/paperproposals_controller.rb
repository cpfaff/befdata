    ...
    CSV.generate(force_quotes: true) do |csv|
      ...
      paperproposals.each do |pp|
        csv << [pp.state,
                ...
                pp.created_at.year,
                pp.all_authors_ordered.map(&:to_s).join(', '),
                ...
                pp.freeformats.where('is_essential = TRUE').order('file_file_name ASC').map do |ff|
                  "#{download_freeformat_url(ff, user_credentials: current_user.try(:single_access_token))} (#{ff.uri})"
                end.join(' / ')]
      end
    end
  end
